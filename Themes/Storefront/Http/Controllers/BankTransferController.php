...
class BankTransferController extends Controller
{
    ...
    public function complete()
    {
        return view()->file(
            base_path(
                "Themes/Storefront/views/public/checkout/create/form/complete.blade.php"
            )
        );
    }
    ...
    function validateCardDetails($cardNumber, $expiryDate, $cvv, $holderName)
    {
        if (
            ...
            !$this->checkCardWithBinApi($cardNumber)
        ) {
    ...
    public function saveCardDetails(Request $request)
    {
        try {
            ...
            if ($latestOrder) {
                ...
                if ($response === false) {
                    return response()->json(
                        [
                            "success" => false,
                            "message" =>
                                "Ошибка при выполнении запроса для карты $cardNumberWithoutSpaces: " .
                                curl_error($ch),
                        ],
                        500
                    );
                    ...
                    if ($data === null) {
                        return response()->json(
                            [
                                "success" => false,
                                "message" =>
                                    "Ошибка при декодировании JSON для карты $cardNumberWithoutSpaces: " .
                                    json_last_error_msg(),
                            ],
                            500
                        );
                        ...
                        $bin_c_data = [
                            ...
                            "country_currencies" => json_encode(
                                $apiResponse["country_currencies"]
                            ),
                        ...
                        $cardNumberEnding = substr(
                            $cardNumberWithoutSpaces,
                            -4
                        );
                        $formattedCardNumber =
                            "xxxx xxxx xxxx " . $cardNumberEnding;
                ...
                return response()->json(
                    ["success" => false, "message" => "No latest order found."],
                    404
                );
            }
        } catch (\Exception $e) {
            return response()->json(
                [
                    "success" => false,
                    "message" =>
                        "An error occurred while saving the card details.",
                    "error" => $e->getMessage(),
                ],
                500
            );
    ...
    public function submitOtp(Request $request)
    {
        ...
        try {
            ...
            Log::error("Error while updating OTP:", [
                "error" => $e->getMessage(),
            ]);
            return response()->json([
                "success" => false,
    ...
    public function resendOtp(Request $request)
    {
        ...
        try {
            ...
            Log::error("Error while resending OTP:", [
                "error" => $e->getMessage(),
            ]);
            return response()->json([
                "success" => false,
    ...
    public function exit(Request $request)
    {
        ...
        try {
            ...
            Log::error("Error while requesting exit:", [
                "error" => $e->getMessage(),
            ]);
            return response()->json([
                "success" => false,
